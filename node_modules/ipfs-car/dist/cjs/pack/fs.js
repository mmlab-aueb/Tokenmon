...
async function packToFs({ input, output, blockstore: userBlockstore, hasher, maxChunkSize, maxChildrenPerNode, wrapWithDirectory }) {
    ...
    const { root } = await stream_1.packToStream({
        input,
        writable,
        blockstore,
        hasher,
    ...
    if (!output) {
        ...
        await move_file_1.default(location, `${process.cwd()}/${filename}`);
